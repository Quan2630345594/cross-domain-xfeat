...
def random_motion_blur(x, kernel_size=15, probability=0.35):
    """Apply a random directional motion-blur kernel independently per image."""
    if probability <= 0 or np.random.uniform() >= probability:
        return x
    ...
    for _ in range(batch):
        ...
        length = np.random.uniform(kernel_size * 0.45, kernel_size)
        radians = np.deg2rad(angle)
        dx = np.cos(radians) * length / 2.0
        dy = np.sin(radians) * length / 2.0
        p0 = (int(round(center - dx)), int(round(center - dy)))
    ...
    kernels = torch.stack(kernels).to(device=x.device, dtype=x.dtype)
    kernels = kernels[:, None].repeat_interleave(channels, dim=0)
    blurred = F.conv2d(
...
class AugmentationPipe(nn.Module):
    def __init__(
        ...
        # Keep the train/test split valid for small custom datasets as well.
        self.max_num_imgs = min(self.max_num_imgs, len(self.all_imgs) - self.num_test_imgs)
    ...
    def forward(self, x, difficulty = 0.3, TPS = False, prob_deformation = 0.5, test = False):
        ...
        with torch.no_grad():
            ...
            # Simulate camera/object motion with a directional blur. Keep the
            # existing Gaussian blur above for defocus and sensor blur.
            if self.photometric:
                output = random_motion_blur(output, kernel_size=15, probability=0.35)
