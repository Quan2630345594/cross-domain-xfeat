...
def parse_arguments():
    ...
    parser.add_argument('--pretrained_weights', type=str, default=None,
                        help='Optional XFeat checkpoint used to initialize fine-tuning.')
...
class Trainer():
    ...
    def __init__(self, megadepth_root_path, 
                       ...
                       save_ckpt_every = 500, pretrained_weights=None):

        self.dev = torch.device ('cuda' if torch.cuda.is_available() else 'cpu')
        self.net = XFeatModel().to(self.dev)

        if pretrained_weights:
            print(f'Loading XFeat pretrained weights: {pretrained_weights}')
            state = torch.load(pretrained_weights, map_location=self.dev)
